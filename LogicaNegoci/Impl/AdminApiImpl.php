...
class AdminApiImpl implements AdminApi {
    ...
    public function actualitzaCampTaula($taula, $camp, $idreg, $valor) {
	 ...
	 public function updateTurnoRotacionalFicha($taula, $camp, $idreg, $valor)
    {

        //METODO PARA ACTUALIZAR IDHORARIS1 O IDHORARIS2
        $this->actualitzaCampTaula($taula, $camp, $idreg, $valor);

        //METODO PARA ASIGNAR LOS TURNOS ROTATIVOS EN ROTACIO

        //GUARDAR DESDE DIA INICIAL DEL AÑO AL DIA FINAL DEL AÑO
        // CAMPOS A GUARDAR: IDEMPLEAT, ID TIPUS TORN, DATE (DATE INICIAL Y DATE FINAL)
        //INSERT

        $year = date('Y'); // Cambia el año según sea necesario
        if (date('L', strtotime("$year-01-01"))) $daysInYear = 366; // Año bisiesto
        else $daysInYear = 365; // Año no bisiesto

        $daysOfYear = [];
        for ($day = 0; $day <= $daysInYear - 1; $day++) $daysOfYear[] = date("Y-m-d", strtotime("$year-01-01 +$day days"));


        //SI EXISTE ALGUN TURNO YA ASIGNADO BORRELO
        $responseRotacio = $this->getDb()->executarConsulta('SELECT * FROM rotacio WHERE idempleat = ' .$idreg .' AND YEAR(data) = ' .$year);

        if(!empty($responseRotacio)) $this->getDb()->executarSentencia('DELETE FROM rotacio WHERE idempleat = ' .$idreg .' AND YEAR(data) = ' .$year);
        
        if($valor != 0) foreach($daysOfYear as $date) $this->getDb()->executarSentencia('insert into rotacio (idempleat,idtipustorn,data) values ('.$idreg.','.$valor.',"'.$date.'")');

    }
	
	 public function actualitzaFestiu($taula, $camp, $idreg, $valor) {

        //CASO HPOTETICO QUE SE ACTUALICE DIA
        $this->actualitzaCampTaula($taula, $camp, $idreg, $valor);
      

        //VAMOS A ACTUALIZAR EL DIA DE DATAANY

        //TOCA TRAER LOS VALORES DE DATAANY DEBIDO A QUE SOLO CONTAMOS CON UN PARAMETRO DE Y-M-D
        $sql_dataany = "SELECT * FROM $taula WHERE idfestiu = $idreg";
        $res = $this->getDb()->executarConsulta($sql_dataany);
        $dataany = $res[0]['dataany'];
        $componentes = explode("-", $dataany);
        $anyDataany = $componentes[0];
        $mesDataany = $componentes[1];
        $diaDataany = $componentes[2];


        //CONDICIONES PARA ACTUALIZAR DATAANY
        if ($camp == 'dia') {
            $valor = $anyDataany.'-'.$mesDataany.'-'.$valor;
            $this->actualitzaCampTaula($taula, 'dataany', $idreg, $valor);
        }
        if ($camp == 'mes') {
            $valor = $anyDataany.'-'.$valor.'-'.$diaDataany;
            $this->actualitzaCampTaula($taula, 'dataany', $idreg, $valor);
        }
    }
    ...
    public function traigoFestivos() {
        $sql = 'SELECT * FROM festiu WHERE idlocalitzacio = 2';
        return $this->getDb()->executarConsulta($sql);
    }
    ...
    public function getTreballadorsIdEmpresaMarcatge($year, $month,$dpt)
    {
		
		if($dpt !== 0)
        {
            $sql = '
        SELECT m.id_emp, m.id_tipus, m.entsort, m.datahora, m.observacions, d.iddepartament, d.nom
FROM marcatges as m
JOIN pertany as p ON p.id_emp = m.id_emp
JOIN departament as d ON d.iddepartament = p.id_dep
        WHERE YEAR(datahora) = ' . $year . ' AND MONTH(datahora) = ' . $month . ' AND d.iddepartament = ' . $dpt . '
        GROUP BY id_emp
        HAVING COUNT(*) > 0
        ';
            return $this->getDb()->executarConsulta($sql);
...
// Cerrar el resultado de la consulta
        ...
        if($chkhrqd==1){
            ...
            while(strtotime($data)<=strtotime($datafi)){
                ...
                if(($chkhrrd==1)) {
                    ...
                    if($rndmin<0){$minsless = " ".$rndmin." minutes";}
                    $datahoraent = date("Y-m-d H:i:s",strtotime($data." ".$horaini.$minsless));
                }
    ...
    public function seleccionaHorarisEmpPerId($id, $any) {
    ...
    public function generaTblTipustorn($idemp){
        ...
        foreach($tornstipus as $t){
            ...
            $btnEdit = '<button class="btn-next btn-small" onclick="mostraEditaTipustorn('.$t["idtipustorn"].');"><span class="glyphicon glyphicon-pencil"></span></button>';
    ...
    public function calculaHoresTreballadesPerIdDia($id, $data) {
        ...
        if(($hores>0)) {$hores = (float)$hores - (float)$hdesc;;}//if($nummarc<=2)&&($hdesc<=2)&&
  ...
  public function assignaExcepcio($id, $idtipusexcep, $datainici, $datafi) {
             ...
             $sqlResponsable ='SELECT idresp FROM empleat WHERE idempleat = '.$id.'   ';
        $resultadoSqlResponsable = $this->getDb()->executarConsulta($sqlResponsable);
        if($resultadoSqlResponsable != null) $resultadoSqlResponsable = 0;

        $sql = 'INSERT INTO excepcio (idempleat, idtipusexcep, datainici, datafinal,idresp) VALUES ('.$id.', '.$idtipusexcep.', "'.$datainici.'", "'.$datafi.'", '.$resultadoSqlResponsable.')';   
        return $this->getDb()->executarSentencia($sql);
    ...
    public function SendEMAILV1($ASUNT,$MSJ,$EMAIL)
    {
        ...
        try {
            ...
            $mail->Username = 'sebastiand.onyx@gmail.com'; // Reemplaza esto con tu dirección de correo de Gmail
            $mail->Password = 'xwxg roml lhzs trur'; // Reemplaza esto con tu contraseña de Gmail
 ...
 ';
    ...
    public function seleccionaExcepcionsEmpPerId($id, $any) {
        $sql = '
    ...
    public function altaPersona($idempresa, $cognom1, $cognom2, $nom, $dni, $datanaix, $numafil, $dpt, $rol, $subemp, $resp) {
        ...
        $nomSinEspacio = str_replace(' ', '', $nom);
        $cognom1SinEspacio = str_replace(' ', '', $cognom1);
        $user =  $nomSinEspacio.$cognom1SinEspacio;
        $sql = 'INSERT INTO empleat (idempresa, cognom1, cognom2, nom, dni, datanaixement, numafiliacio, dataultcontrac, enplantilla, idsubempresa, idresp, idtargeta,user) VALUES ('.$idempresa.',"'.$cognom1.'","'.$cognom2.'","'.$nom.'","'.$dni.'","'.$datanaix.'","'.$numafil.'",now(),1,'.$subemp.','.$resp.',"'.$idtargeta.'","'.$user.'")';
        $this->getDb()->executarSentencia($sql);
    ...
    public function seleccionaHoresTeoriques($id, $any) {
        ...
        if (empty($hours_date)) {
            ...
            $sql = "
            select   h.nom as nom_h ,   t.horestreball as hours, q.datainici as start_day, q.datafi as end_day, t.diasetmana as week_day
            from torn as t
                join horaris as h on h.idhoraris = t.idhorari
                join quadrant as q on q.idhorari = h.idhoraris
            ...
            if($rs[0]['end_day'] === null)
            {
              ...
              //consulta para los horarios formateada para que no traiga tantos datos si no solo dias y horas
                $sql_torn = "
                select  h.nom as nom_h ,   t.horestreball as hours, q.datainici as start_day, q.datafi as end_day, t.diasetmana as week_day
                from torn as t
                    join horaris as h on h.idhoraris = t.idhorari
                ...
                ORDER BY start_day DESC
                ";
                $rs_torn = $this->getDb()->executarConsulta($sql_torn);
               
            

                //RECORRER TODA LA DATA PARA SACAR UNO A UNO LOS DIAS CON LAS HORAS QUE TRAIGA EL HORARIO
                    foreach ($rs_torn as $datos_horario) {  

                            $current_date = strtotime($datos_horario['start_day']);
                            $end_date_timestamp = strtotime($datos_horario['end_day']);
                        
                            while ($current_date <= $end_date_timestamp) {
                                // Obtiene el número del día de la semana (0 para domingo, 1 para lunes, ..., 6 para sábado)
                                

                                $fecha_actual_comparar = new DateTime();
                                $fecha_actual_comparar->setTimestamp($current_date);
                                $dia_semana = $fecha_actual_comparar->format('N');

                               
                                //COMPARA SI EL DIA ES SABADO O DOMINGO PARA EXCLUIRLO
                                    if( $dia_semana != 6 && $dia_semana != 7){
                                    $fecha_actual= date('Y-m-d', $current_date);
                                    $fechas_horas_intervalo[$fecha_actual] = array( $fecha_actual =>  $datos_horario['hours']);
                                    }
            ...
            //ESTA ES LA DATA DE LOS DIAS CON LAS HORAS DEL HORARIO
            $hours_date= $fechas_horas_intervalo ;   
   ...
   public function monthlyEmployeeCalendarMonth($id, $any, $month, $lng) {
        ...
        if (empty($rs))
        {
            ...
            if($rs[0]['end_day'] === null)
            {
                ...
                from torn as t
                    join horaris as h on h.idhoraris = t.idhorari
                    join quadrant as q on q.idhorari = h.idhoraris
                where idempleat = $id and (($any BETWEEN YEAR(q.datainici) AND YEAR(q.datafi) )||(
                YEAR(q.datainici)>=0 and q.datafi IS NULL))
                GROUP BY(idquadrant)
                ";
    ...
    public function seeException ($idexcep, $idtipusexcep) {
	 ...
	 public function days_vacation_any_anterior($id)
    {
        $year = date("Y");
        $before_year = $year - 1;

        $sql = "
        SELECT dias_any_anterior
        FROM vacances 
        ...
        if (!empty($rs)) {
            foreach ($rs as $value) {
                $days_vacation[$year] = $value['dias_any_anterior'];
            }
        } else {
        ...
        if (!empty($rs_before_year)) {
            foreach ($rs_before_year as $value) {
                $days_vacation[$before_year] = $value['dias_any_anterior'];
            }
        } else {
   ...
   public function days_exception($id, $any, $dateParams)
    {
        //TRAIGO LOS DIAS EXCEPCION
        $sql_exception = "
        SELECT e.datainici as start_day, e.datafinal as end_day, t.nom as nom, t.r as r, t.g as g, t.b as b, t.idtipusexcep as idtipusexcep, e.idexcepcio as idexcepcio 
        ...
        if (!empty($rs_exception)) {
            ...
            foreach ($exception_days_interval as $e_d_i) {
                ...
                if ($e_d_i['start_day'] !== $e_d_i['end_day']) {
                    ...
                    //     $dateValue = $exception_day['day'];
                    //     $formatDate = new DateTime($dateValue);
                    //     $month_excepcio = $formatDate->format('m');
            ...
            $exception_days_to_date = [];

            //RECORREMOS LOS EXCEPCION DIAS Y SACAMOS DE AHI SOLO LOS QUE SON HASTA LA FECHA
            foreach ($exception_days as $key => $exception_day) {
            

                $date_exception = new DateTime($exception_day['day']);
                $date_params = new DateTime($dateParams);

                //VALIDAR LOS DATOS MENORES A LA FECHA
                if($date_exception <= $date_params)  $exception_days_to_date[$key] = $exception_day;
            ...
            foreach ($exception_days_to_date as $exception_day) {
    ...
    public function dataSpecialPeriodsReport ($idsubemp, $dpt, $tipusexcep, $idempresa, $mes, $any, $periodo, $empleat)
    {

        //TOMA DE LOS AÑOS Y MESES PARA CONTAR LOS DIAS POR ESTOS PERIODOS
        $periods = [];
        $periods[] = ['year' => $any, 'month' => $mes];
        $numMonthsToShow = $this->getNumMonthsToShow($periodo);

        for ($i = 1; $i < $numMonthsToShow; $i++) {
            $mes--;
            if ($mes == 0) {
                $mes = 12;
                $any--;
        ...
        if ($tipusexcep == "Tots") return ['employees_periods' => []];
        ...
        if ($empleat != "Tots")
        {
            if ($idsubemp != "Totes" && $dpt != "Tots") {
                $sql = 'SELECT DISTINCT e.idempleat, e.nom, e.cognom1, e.cognom2 
                    FROM empleat AS e 
                        JOIN pertany AS p ON p.id_emp = e.idempleat 
                        JOIN departament AS d ON d.iddepartament = p.id_dep 
                    WHERE e.idempleat = "'.$empleat.'" AND e.idempresa = "'.$idempresa.'" AND e.idsubempresa = '.$idsubemp.' AND d.nom = "'.$dpt.'" ORDER BY e.cognom1, e.cognom2, e.nom AND e.enplantilla = 1';
            ...
            if ($idsubemp != "Totes" && $dpt == "Tots") {
                $sql = 'SELECT * 
                    FROM empleat 
                    WHERE e.idempleat = "'.$empleat.'" AND idempresa = "'.$idempresa.'" AND idsubempresa = '.$idsubemp.' AND enplantilla = 1
            ...
            if ($idsubemp == "Totes" && $dpt != "Tots") {
                $sql = 'SELECT DISTINCT e.idempleat, e.nom, e.cognom1, e.cognom2 
                    FROM empleat AS e 
                        JOIN pertany AS p ON p.id_emp = e.idempleat 
                        JOIN departament AS d ON d.iddepartament = p.id_dep 
                    WHERE e.idempleat = "'.$empleat.'" AND d.nom = "'.$dpt.'" ORDER BY e.cognom1, e.cognom2, e.nom AND enplantilla = 1';
                $persones = $this->getDb()->executarConsulta($sql);
            }
            ...
            if ($idsubemp == "Totes" && $dpt == "Tots") {
                $sql = 'SELECT e.idempleat, e.nom, e.cognom1, e.cognom2 
                    FROM empleat AS e
                    WHERE e.idempleat = "'.$empleat.'" AND e.enplantilla = 1
                    ORDER BY cognom1, cognom2, nom';
                $persones = $this->getDb()->executarConsulta($sql);
            }
        }
        ...
        if ($empleat == "Tots")
        {
            if ($idsubemp != "Totes" && $dpt != "Tots") {
                $sql = 'SELECT DISTINCT e.idempleat, e.nom, e.cognom1, e.cognom2 
                    FROM empleat AS e 
                        JOIN pertany AS p ON p.id_emp = e.idempleat 
                        JOIN departament AS d ON d.iddepartament = p.id_dep 
                    WHERE e.idempresa = "'.$idempresa.'" AND e.idsubempresa = '.$idsubemp.' AND d.nom = "'.$dpt.'" ORDER BY e.cognom1, e.cognom2, e.nom AND e.enplantilla = 1';
                $persones = $this->getDb()->executarConsulta($sql);
            }
            ...
            if ($idsubemp != "Totes" && $dpt == "Tots") {
                $sql = 'SELECT * 
                    FROM empleat 
                    WHERE idempresa = "'.$idempresa.'" AND idsubempresa = '.$idsubemp.' AND enplantilla = 1
                    ORDER BY cognom1, cognom2, nom';
                $persones = $this->getDb()->executarConsulta($sql);
            }
            ...
            if ($idsubemp == "Totes" && $dpt != "Tots") {
                $sql = 'SELECT DISTINCT e.idempleat, e.nom, e.cognom1, e.cognom2 
                    FROM empleat AS e 
                        JOIN pertany AS p ON p.id_emp = e.idempleat 
                        JOIN departament AS d ON d.iddepartament = p.id_dep 
                    WHERE d.nom = "'.$dpt.'" ORDER BY e.cognom1, e.cognom2, e.nom AND enplantilla = 1';
                $persones = $this->getDb()->executarConsulta($sql);
            }
        ...
        foreach ($persones as $persone)
        {
            ...
            {
                if ($exception['idempleat'] === $persone['idempleat']) $employee_exception[] = [
                    'idempleat' => $persone['idempleat'],
                    'nom' => $persone['nom'],
                    'cognom1' => $persone['cognom1'],
                    'cognom2' => $persone['cognom2'],
                    'start_day' => $exception['start_day'],
                    'end_day' => ($exception['end_day'] !== '0000-00-00') ? $exception['end_day'] : $exception['start_day']
                ];
            }

            if (empty($employee_exception)) $employee_exception[] = [
                'idempleat' => $persone['idempleat'],
                'nom' => $persone['nom'],
                'cognom1' => $persone['cognom1'],
                'cognom2' => $persone['cognom2'],
        ...
        foreach ($data_employee as $employee)
        {
            ...
            $employee_periods['idempleat'] = $employee[0]['idempleat'];
            $employee_periods['Persona/Mes'] = $employee[0]['cognom1'] .' ' .$employee[0]['cognom2'] .', ' .$employee[0]['nom'];
            ...
            {
                if ($key !== 'Persona/Mes' && $key !== 'idempleat') $employee_periods[$key] = array_sum($employee_period);
            }
        ...
        return $employees_periods;
    }

    public function headerSpecialPeriodsReport ($mes, $any, $periodo)
    {
    ...
    private function getNumMonthsToShow ($periodo)
    {
        switch ($periodo) {
            case 'anual':
                return 12;
    ...
    public function allDataSpecialPeriodsReport ($idsubemp, $dpt, $idempresa, $mes, $any, $periodo, $empleat)
    {
        $tipus_excepcio_array = [];
        $tipus_excepcio_array = $this->getTipusException();

        $all_exception = [];
        $people = [];
        foreach ($tipus_excepcio_array as $tipus_excepcio) {
            $employees_periods = $this->dataSpecialPeriodsReport($idsubemp, $dpt, $tipus_excepcio['idtipusexcep'], $idempresa, $mes, $any, $periodo, $empleat);

            $all_exception[$tipus_excepcio['nom']] = $employees_periods;

            foreach ($employees_periods as $key => $employee_period) {
                if(empty($people[$employee_period['idempleat']])) $people[$employee_period['idempleat']] = ['Persona/Mes' => $employee_period['Persona/Mes']];
            }

        }

        foreach ($people as $idempleat => $person)
        {
            foreach ($all_exception as $name_exception =>$exception)
            {
                foreach ($exception as $empleat_exception)
                {
                    if ($empleat_exception['idempleat'] == $idempleat) $people[$idempleat][$name_exception] = [
                            'value' => $empleat_exception['Total_Dies'] ,
                            'data' => $empleat_exception
                        ];
                }
            }
        }
    ...
    public function allHeaderSpecialPeriodsReport($mes, $any)
    {
        //CREO CABECERA
        $header = [];
        $header[] = "Persona \ Mes";
        $tipus_excepcio_array = $this->getTipusException();
        foreach ($tipus_excepcio_array as $tipus_excepcio) $header[] = $tipus_excepcio['nom'];
        return $header;
    }

    public function getTipusException ()
    {
        return $rs = $this->getDb()->executarConsulta("SELECT * FROM tipusexcep");
    }
	...
	public function tiempoMarcajeAutoMes ($id, $mes, $año, $diafi)
    {
        //PRI FESTIUS
        $sql_company = "SELECT prifestius, marcafestius FROM empresa WHERE idempresa = 7";
        $rs_company = $this->getDb()->executarConsulta($sql_company);
        foreach ($rs_company as $valor)
        {
            $prifestius = $valor['prifestius'];
        ...
        //TRAIGO EL TURNO EL HORARIO DEL EMPLEADO
        $sql = "
            select t.horestreball as horasTrabajadas, q.datainici as start_day, q.datafi as end_day, t.horaentrada as horaEntrada, t.horasortida as horaSalida, t.diasetmana as week_day
            from torn as t
                join horaris as h on h.idhoraris = t.idhorari
                join quadrant as q on q.idhorari = h.idhoraris
                join empleat as e on q.idempleat = e.idempleat
            where e.idempleat = $id and (($año BETWEEN YEAR(q.datainici) AND YEAR(q.datafi) )||( YEAR(q.datainici)>=0 and q.datafi IS NULL)) AND q.actiu = 1
            ";
        $rs = $this->getDb()->executarConsulta($sql);

        //CREO FECHA INICIAL DE LOS PARAMETROS
        $octoberDate = new DateTime($año . '-' . $mes . '-01');
        $data = [];
        $total_work_seconds = 0;

        //TRAIGO LOS DIAS EXCEPCION
        $sql_exception = "
            SELECT e.datainici as start_day, e.datafinal as end_day, t.nom as nom
            FROM excepcio as e
                JOIN tipusexcep as t ON t.idtipusexcep = e.idtipusexcep
            WHERE e.idempleat = $id AND YEAR(e.datainici) = $año AND MONTH(e.datainici) = $mes AND YEAR(e.datafinal) = $año AND MONTH(e.datafinal) = $mes AND ((e.idresp = 0 OR  e.idresp IS NULL) OR e.aprobada = 1)
            ";
        $rs_exception = $this->getDb()->executarConsulta($sql_exception);

        //OPERACION PARA EXTRAER LOS DIAS DE LOS INTERVALOS DE DIAS EXCEPCION
        if (!empty($rs_exception)) {
            $exception_days_interval = [];

            foreach ($rs_exception as $valor) $exception_days_interval[] = [
                'start_day' => $valor["start_day"],
                'end_day' => $valor["end_day"],
                'nom' => $valor["nom"]
            ];
            ...
            foreach ($exception_days_interval as $e_d_i) {
                ...
                if ($e_d_i['start_day'] !== $e_d_i['end_day']) {
                    while ($current_date <= $end_date_timestamp) {
                        $exception_days[] = ['day' => date('Y-m-d', $current_date), 'nom' => $e_d_i['nom']];
                        $current_date = strtotime('+1 day', $current_date);
                    }
                } else {
                    $exception_days[] = ['day' => $e_d_i['start_day'], 'nom' => $e_d_i['nom']];

                }
            }
        }

        //TRAIGO LOS DIAS FESTIVOS
        $sql_holiday = "
        SELECT f.dataany as days, l.nom as nom, f.descripcio as descripcio
        FROM festiu AS f
            join localitzacio as l on l.idlocalitzacio = f.idlocalitzacio
            join situat as s on s.idlocalitzacio = l.idlocalitzacio
            join empleat as e on e.idEmpleat = s.idempleat
        where YEAR(f.dataany) = $año AND YEAR(s.datainici) = $año AND YEAR(s.datafi) = $año
        ";
        $rs_holiday = $this->getDb()->executarConsulta($sql_holiday);

        for ($i=1; $i <= $diafi; $i++)
        {
            //OBETENGO DIA DE LA SEMANA
            $dayOfWeek =  $octoberDate->format('N');

            //OBTENGO DIA DEL MES
            $dayOfMonth = $octoberDate->format('d');

            $fechaFormateada = $octoberDate->format('Y-m-d');

            //VERIFICAMOS SI ESTA EN DIAS DE EXCEPCION
            $make_day = true;
            if (!empty($rs_exception))
            {
                foreach ($exception_days as $exception_day)
                    if ($exception_day['day'] === $fechaFormateada) {
                        $make_day = false;
                        break;
                    }
            }

            //VERIFICAMOS SI ESTA EN DIAS FESTIVOS
            if( (!empty($rs_holiday)) && ($prifestius==1) )
            {
                foreach ($rs_holiday as $valor)
                    if ($valor["days"] === $fechaFormateada) {
                        $make_day = false;
                        break;
                    }
            }
            ...
            if ($make_day)
            {
                //CAPTURO EL DIA DEL TURNO
                foreach ($rs as $item)
                {
                    if ($item['week_day'] === $dayOfWeek)
                    {
                        //CALCULO LAS HORAS TRABAJADAS
                        $work_hours_format_hour = $this->decimalsToTimeFormat($item['horasTrabajadas']);
                        $work_hours_random = $this->generateHourRandom($work_hours_format_hour);
                        $start_time = $this->generateHourRandom($item['horaEntrada']);
                        $end_time = $this->generateHourRandom($item['horaSalida']);

                        //TOMO LOS SEGUNDOS TRABAJADOS PARA EL GRAN TOTAL
                        $work_hours_array = explode(':', $work_hours_random);
                        $seconds = 0;
                        $seconds = ($work_hours_array[0] * 3600) + ($work_hours_array[1] * 60);
                        $total_work_seconds += $seconds;

                        $data[] = [
                            'dayOfMonth' => $dayOfMonth,
                            'start_time' => ($item['horasTrabajadas'] != 0) ? $start_time : '',
                            'end_time' => ($item['horasTrabajadas'] != 0) ? $end_time : '',
                            'work_hours' => ($item['horasTrabajadas'] != 0) ? $work_hours_random : '',
                            'observations' => ''
                        ];
                    }
                }
            } else {
                //APLICAMOS EN LAS OBSERVACIONES LOS NOM DE EXCEPCION O DE FESTIVO
                $observations = '';
                if (!empty($rs_exception))
                {
                    foreach ($exception_days as $exception_day)
                        if ($exception_day['day'] === $fechaFormateada) {
                            $observations = $exception_day['nom'];
                            break;
                        }
                }
            ...
            $octoberDate = $octoberDate->modify('+1 day');
        }


        // Calcular horas y minutos
        $totalHoras = floor($total_work_seconds / 3600);
        $totalMinutos = floor(($total_work_seconds % 3600) / 60);

        // Formatear el resultado
        $total_hours = sprintf('%02d:%02d', $totalHoras, $totalMinutos);
	...
	private function generateHourRandom ($time)
    {
        ...
        $random_difference = rand(-10, 10);
        // Aplicar la diferencia aleatoria a las horas y minutos
        $adjusted_hours = $hours;
        $adjusted_minutes = $minutes + $random_difference;
        // Asegurarse de que los minutos estén en el rango de 0 a 59
        $adjusted_minutes = ($adjusted_minutes < 0) ? 0 : (($adjusted_minutes > 59) ? 59 : $adjusted_minutes);

        // Formatear las horas y minutos ajustados
        return sprintf('%02d:%02d', $adjusted_hours, $adjusted_minutes);
    }


    public function tiempoMarcajeAutoMesTotal($mes, $año, $diafi)
    {
        $tiempoMarcajeAutoMesTotal = [];
        $sql = "SELECT idempleat FROM empleat ORDER BY cognom1";
        $rs = $this->getDb()->executarConsulta($sql);
        if ( !empty($rs) )
            foreach ($rs as $value)
                $tiempoMarcajeAutoMesTotal[] = $this->tiempoMarcajeAutoMes($value['idempleat'], $mes, $año, $diafi);

        return $tiempoMarcajeAutoMesTotal;
    }
    ...
    public function getIdEmployees ($idsubempresa)
    {
        if ($idsubempresa != 'Totes') $sql = "
        SELECT DISTINCT e.idempleat, e.cognom1, e.cognom2, e.nom, e.numafiliacio, e.email
        FROM empleat AS e
         JOIN assignat AS a ON a.id_emp = e.idempleat
         JOIN rol AS ro ON ro.idrol = a.id_rol
         LEFT JOIN pertany AS p ON p.id_emp = e.idempleat
         LEFT JOIN departament AS d ON d.iddepartament = p.id_dep
        WHERE ro.esempleat = 1 AND a.actiu = 1 AND e.enplantilla = 1 AND idsubempresa = $idsubempresa
        ORDER BY e.numafiliacio
        ";
        ...
        FROM empleat AS e
         ...
         LEFT JOIN pertany AS p ON p.id_emp = e.idempleat
         LEFT JOIN departament AS d ON d.iddepartament = p.id_dep
        WHERE ro.esempleat = 1 AND a.actiu = 1 AND e.enplantilla = 1
        ORDER BY e.numafiliacio
        ";
    ...
    public function exceptionByIdByDate ($idempleat, $tipusException, $date)
    {
        //TRAIGO LOS DIAS EXCEPCION

        if($tipusException != 'id_ausencia' && $tipusException != 'Tots')
        {
            $sql_exception = "
            SELECT e.idempleat, e.datainici as start_day, e.datafinal as end_day, t.nom, t.idtipusexcep.
            FROM excepcio as e
                JOIN tipusexcep as t ON t.idtipusexcep = e.idtipusexcep
            WHERE e.idempleat = $idempleat AND t.idtipusexcep = $tipusException AND ((e.idresp = 0 OR  e.idresp IS NULL) OR e.aprobada = 1) 
            ";
            $rs_exception = $this->getDb()->executarConsulta($sql_exception);
        } else {
            $sql_exception = "
        ...
        FROM excepcio as e
            JOIN tipusexcep as t ON t.idtipusexcep = e.idtipusexcep
        WHERE e.idempleat = $idempleat AND ((e.idresp = 0 OR  e.idresp IS NULL) OR e.aprobada = 1) 
        ";
            $rs_exception = $this->getDb()->executarConsulta($sql_exception);
        }

        //SEPARO POR INTEVARLO LAS EXCEPCION
        $exceptions = [];
        foreach ($rs_exception as $item)
        {
            $current_date = strtotime($item['start_day']);
            $end_date_timestamp = strtotime($item['end_day']);
            ...
            if ($item['start_day'] !== $item['end_day'])
            {
                while ($current_date <= $end_date_timestamp) {
                    $exceptions[] = [
'idtipusexcep' => $item['idtipusexcep'],
                        'idempleat' => $item['idempleat'],
                        'nom' => $item['nom'],
                        'day' => date('Y-m-d', $current_date),
                    ];
...
'idtipusexcep' => $item['idtipusexcep'],
        ...
        //RECORREMOS LAS FECHAS Y VERIFICAMOS SI LA FECHA ESTA EN LAS EXCEPCIONES
        $observation = [
            'idtipusexcep' =>  '', 'nom' => ''
        ];
        foreach ($exceptions as $exception) if ($exception['day'] == $date) {
            
            $observation = ['idtipusexcep' =>  $exception['idtipusexcep'], 'nom' => $exception['nom']];
            break;
    ...
    public function summarySpecialPeriods ($dateEnd, $dateStart, $idsubemp)
    {
        

        //year
        $any = date('Y', strtotime($dateEnd));
        

        //TRAIGO LOS DIAS EXCEPCION
        if ($idsubemp != 'Totes') $sql_exception = "
        SELECT e.datainici as start_day, e.datafinal as end_day, t.nom as nom, t.r as r, t.g as g, t.b as b, t.idtipusexcep as idtipusexcep, e.idexcepcio as idexcepcio 
        FROM excepcio as e
            JOIN tipusexcep as t ON t.idtipusexcep = e.idtipusexcep
            JOIN empleat as emp On emp.idempleat = e.idempleat
            JOIN assignat AS a ON a.id_emp = emp.idempleat
            JOIN rol AS ro ON ro.idrol = a.id_rol
        WHERE YEAR(e.datainici) = $any AND (YEAR(e.datafinal) = $any) AND ((e.idresp = 0 OR  e.idresp IS NULL) OR e.aprobada = 1) AND ro.esempleat = 1 AND a.actiu = 1 AND emp.enplantilla = 1 AND emp.idsubempresa = $idsubemp
        ";

        if ($idsubemp == 'Totes') $sql_exception = "
        SELECT emp.idempleat as idempleat, e.datainici as start_day, e.datafinal as end_day, t.nom as nom, t.r as r, t.g as g, t.b as b, t.idtipusexcep as idtipusexcep, e.idexcepcio as idexcepcio, emp.nom as nom_emp
        FROM excepcio as e
            JOIN tipusexcep as t ON t.idtipusexcep = e.idtipusexcep
            JOIN empleat as emp On emp.idempleat = e.idempleat
            JOIN assignat AS a ON a.id_emp = emp.idempleat
            JOIN rol AS ro ON ro.idrol = a.id_rol
        WHERE YEAR(e.datainici) = $any AND (YEAR(e.datafinal) = $any) AND ((e.idresp = 0 OR  e.idresp IS NULL) OR e.aprobada = 1) AND ro.esempleat = 1 AND a.actiu = 1 AND emp.enplantilla = 1
        ";
        ...
        if (!empty($rs_exception))
        {
            ...
            foreach ($rs_exception as $valor) {
                ...
                $exception_days_interval[] = [
                    'idempleat' => $valor["idempleat"],
                    'nom_emp' => $valor["nom_emp"],
                    'start_day' => $valor["start_day"],
                    'end_day' => $valor["end_day"],
                    'type_exception' => $valor["nom"],
                    'rgb' => $rgb,
            ...
            foreach ($exception_days_interval as $e_d_i)
            {
                ...
                if ($e_d_i['start_day'] !== $e_d_i['end_day'])
                {
                    while ($current_date <= $end_date_timestamp) {
                        $exception_days[] = [
                            'idempleat' => $e_d_i["idempleat"],
                            'nom_emp' => $e_d_i["nom_emp"],
                            'day' => date('Y-m-d', $current_date),
                            'type_exception' => $e_d_i["type_exception"],
                            'rgb' => $e_d_i["rgb"],
                            'idtipusexcep' => $e_d_i['idtipusexcep'],
            ...
            $exception_days_start_end = [];
            //SACO LOS DIAS DE EXCEPCION ENTRE DATE END Y DATE START
            foreach ($exception_days as $exception_day)
            {
                if ($exception_day['day'] >= $dateStart && $exception_day['day'] <= $dateEnd) $exception_days_start_end[] = $exception_day;
            }
            ...
            // NECESITO SEPARAR LOS EXCEPCION DAYS POR TIPO DE EXCEPCION

            //TOMO LOS ID EXCEPCIO
            $key_tipusexcept = [];
            foreach ($exception_days_start_end as $exception_day)   $key_tipusexcept[] =  $exception_day["idtipusexcep"]; 

            //DEJO LOS UNIQUES
            $unique_key_tipusexcep = array_unique($key_tipusexcept);
            


            $groupByTipusExcep = [];
           
            //CREO ARRAY DE ELEMENTOS EN EL QUE CADA ELEMENTO TIPO EXCEPCION ESTEN TODOS LOS DIAS DE ESE EXCEPCION
            foreach($exception_days_start_end as $all_days)
            {
                foreach($unique_key_tipusexcep as $key_tipusexcep)
                {
                    if($key_tipusexcep == $all_days["idtipusexcep"]) $groupByTipusExcep[$key_tipusexcep][] = $all_days;
                }
            }

            //CREO ARRAY AGRUPACION DE POR TIPUS EXCEPCION, Y EN CADA TIPUS EXCEPCION SE APLICA LOS UNIQUES EMPLEAT
            $group_exceptcion_unique_empleat = [];
           // print_r($groupByTipusExcep);
            echo' <br>';
            foreach($groupByTipusExcep as $key => $tipus_excep) $group_exceptcion_unique_empleat[$key][]  = $this->unique_multidim_array($tipus_excep, 'idempleat');
            

            

            foreach($group_exceptcion_unique_empleat as $key => $tipusexcep)
            {
                
                // CUENTO LOS DIAS EXCEPCION POR TIPO, PRIMERO CREO LOS UNIQUES
              
                $uniques_excepcio[$key] = [
                    'type_excepcio' => $tipusexcep[0][0]['type_exception'],
                    'rgb' => $tipusexcep[0][0]['rgb'],
                    'count_days' => count($tipusexcep[0])
                ];
            }
        } 
    ...
    private function unique_multidim_array($array, $key) {
	    ...
	    public function addMasArchivos ($carpeta, $file)
    {
        $root = $_SERVER["DOCUMENT_ROOT"];
        ...
        if (count($file) > 0) {
            ...
            $pathFoler = $micarpeta.$sep.$carpeta;

            if (!file_exists($pathFoler)) $response2 = mkdir($pathFoler, 0777, true);
    ...
    public function borrarArchivo ($carpeta, $namefile)
    {
        $root = $_SERVER["DOCUMENT_ROOT"];
        if (strpos($root, '/') === false) $sep = "\\";
        else $sep = "/";
        $pathArchivo = $root . $sep . 'excepciFiles' . $sep . $carpeta . $sep . $namefile;
        if (file_exists($pathArchivo)) unlink($pathArchivo);
    }

	 public function getSolicitudPeriodos($FilterTable, $lng, $id)
    {

        //DATA
        $result = $this->getDb()->executarConsulta(
            'SELECT excepcio.idresp,excepcio.idtipusexcep,excepcio.idexcepcio, empleado.nom as empleatnom, tipusexcep.nom as tipusexcepnom , datainici , datafinal'
            . ' , excepcio.observacions ,empleRespns.cognom1 as empleRespnscognom1 ,empleRespns.cognom2 as empleRespnscognom2, empleRespns.nom as empleRespnsnom , empleado.cognom1 as empleadocognom1, empleado.cognom2 as empleadocognom2, aprobada , dataaprovacio , excepcio.idempleat,tipusexcep.requereixFitxer as requiereArchivo '
        ...
        foreach ($result as $row) {
            ...
            $idexcep = $row["idexcepcio"];
            $estado_aprobacion = $row["aprobada"];
            $requiereArchivo= $row["requiereArchivo"];

            //VERIFICAR EL ESTADO DE LA SOLICITUD
            $stateExept = $this->verificarEstadoSolicitud($estado_aprobacion, $lng);

            //VERIFICAR SI REQUIERE ARCHIVOS LA SOLICITUD
            $mostrarEnTabla = $this->requiereArchivosSolicitudPeriodo($idexcep, $requiereArchivo);

            //CREACION TABLA
            $tblpers .=  "<tr style='background:$mostrarEnTabla[color];'>"
                . "<td class='fila' >" . $row["empleatnom"] . " " . $row["empleadocognom1"] . " " . $row["empleadocognom2"] .   "</td>"
                . "<td class='fila'>" . $this->__($lng, $row["tipusexcepnom"]) . "</td>"
                . "<td class='fila'>" . $row["datainici"] . "</td>"
                . "<td class='fila'>" . $row["datafinal"] . "</td>"
                . "<td class='fila'>" . $stateExept . "</td>";
            $tblpers .= "<td class='fila'> $mostrarEnTabla[text]  </td>";
            $tblpers .= "<td class='fila'>" . $row["empleRespnsnom"] . " " . $row["empleRespnscognom1"] . " " . $row["empleRespnscognom2"] . "    </td>";
            $tblpers .= '<td style="background-color:$mostrarEnTabla[color] "> 
    ...
    private function requiereArchivosSolicitudPeriodo($idexcep, $requiereArchivo)
    {
        $coloryMostrar=[
            "text"=> "",
            "color"=>""
        ...
        //SI NO ES NECESARIA RETORNAR
        if($requiereArchivo == 0) {
            $coloryMostrar["text"] = "Archivo No Necesario";
            $coloryMostrar["color"]="#f5f5f5";
            return $coloryMostrar;
        }


        //SI ES NECESARIO BUSCANDO SI HYA ARCHIVOS
        if($requiereArchivo == 1)
        {
            $coloryMostrar["text"] = 'Pendiente de subir Comprobante';
            $coloryMostrar["color"]="#F0B27A  ";
            ...
            if (file_exists($pathFoler)) {
                $listFiles = array_values(array_diff(scandir($pathFoler), array('..', '.')));
                if (count($listFiles) > 0){   $coloryMostrar["text"] = 'Comprobante Subido';   $coloryMostrar["color"]="#82E0AA";}
            }
    ...
    private function verificarEstadoSolicitud($estado_aprobacion, $lng)
    {
        $stateExept = $this->__($lng, "pendiente");
        if ($estado_aprobacion == '1') $stateExept = $this->__($lng, "aprobada");
        else if ($estado_aprobacion == '0') $stateExept = $this->__($lng, "Denegada");
        return $stateExept;
    }
...
GROUP BY t1.idempleat, DATE(t1.datahora) ORDER BY e.numafiliacio";
    ...
    public function seleccionaPersonaParaEmail(){
        $sql = 'select e.idempleat, e.cognom1, e.cognom2, e.nom, e.email from empleat as e where e.enplantilla = 1   order by e.cognom1';
        $rs = [];
        $rs = $this->getDb()->executarConsulta($sql);
        //SOLO ENVIO LOS QUE EL CORREO ES DISTINTO A NULL
        foreach ($rs as $key => $item) if($item['email'] == null)  unset($rs[$key]);
        return $rs;
    }

    public function updateMailCompany($correo, $idemp) {
        $this->actualitzaCampTaula('empresa', 'emailindex', $idemp, $correo);
    }

    public function sendMailApi ($idemp, $lng){
        
        //VER QUE FECHA SE REALIZO EL ULTIMO RELLENO

        $res = "";
        $rsautomar = $this->getDb()->executarConsulta('select idempleat, nom, cognom1 from empleat where idempresa='.$idemp);

        foreach($rsautomar as $r){

                $id = $r["idempleat"];
                $name = $r["nom"] .' ' .$r["cognom1"];
                $res.=$id." ";
                $data = "";
                $year = Date('Y');
                ...
                $fecha_anterior = $fecha_actual->format('Y-m-d');
    
                //RECORRO LOS MARCAJES DESDE LA FECHA ANTERIOR
    
                //TRAEMOS LOS DIAS LABORALES DE LOS HORARIOS O DE LOS TURNOS DE ESTE AÑO
                $horario = $this->horarioOturnoEmpleat($id, $year, $lng, $fecha_anterior);
                if(!empty($horario)) 
                {
                    //YA TENGO TODOS LOS DIAS DEL AÑO DEL HORARIO HASTA LA FECHA ACTUAL
                    // ENTONCES REVISO SI PARA ESTOS DIAS HAY MARCAJES DE ESTA PERSONA Y LOS ALMACENO
        
                    //PARA OPTIMIZAR, TRAEMOS LOS MARCAJES DESDE UN INTERVALO DE TIEMPO FIRST_DAY AND END_DAY, PARA NO TRAERLO UNO A UNO
                    $first_day = reset($horario);
                    $last_day = end($horario);
        
                    $first_day = $first_day['day'];
                    $last_day = $last_day['day'];
                    
                    $sql = "SELECT datahora, entsort FROM marcatges WHERE id_emp= $id AND DATE(datahora) >= '$first_day' AND DATE(datahora) <= '$last_day' ";
                    $response_db = $this->getDb()->executarConsulta($sql);
    
                    //GUARDAMOS LOS MARCAJES EN SUS RESPECTIVOS DIAS
                    foreach($horario as $key_day => $day)
                    {
                        foreach($response_db as $marcaje_db)
                        {
                            $date_marcaje = new DateTime($marcaje_db['datahora']);
                            $day_marcaje = $date_marcaje->format('Y-m-d');
                            if($day['day'] == $day_marcaje) $horario[$key_day]['marcajes'][] = ['day' => $marcaje_db['datahora'], 'entsort' => $marcaje_db['entsort']];
                        }
                    }
    
                     // SI NO HAY MARCAJE NO HACER NADA
                    // SI HAY MARCAJES DE ESE DIA PERO FALTAN MARCAJES DE ENTRADA O DE SALIDA SE LOS HACEMOS (MARCAJE ENTRADA O MARCAJE SALIDA)
    
                    foreach($horario as $day) {
    
                        //VERIFICAMOS SI ESE DIA TIENE MARCAJES
                        if(isset($day['marcajes'])) 
                        {
                            $entradas = 0;
                            $salidas = 0;
                            //REVISAMOS CANTIDAD MARCAJES ENTRADA Y CANTIDAD MARCAJES SALIDA
                            foreach($day['marcajes'] as $marcaje)
                            {
                                if($marcaje['entsort'] == 0) $entradas++;
                                if($marcaje['entsort'] == 1) $salidas++;
                            }
    
                            //SI MISMA CANTIDAD MARCAJES ENTRADA ES IGUAL A MISMA CANIDAD MARCAJES SALIDA NO HACER NADA
                            //SI CANTIDAD MARCAJES ENTRADA Y SALIDA DISTINTOS, HACER MARCAJES FALTANTES
                            if($entradas !== $salidas)
                            {
                                $entradas_aux = $entradas;
                                $salidas_aux = $salidas;
    
                                //RELLENAMOS LAS ENTRADAS O SALIDAS HASTA QUE LOS VALORES SEAN IGUALES
                                $observacions = 'Relleno por falta de marcaje';
                                
                                while($salidas_aux < $entradas)
                                {
                                    $diferencia = $entradas - $salidas_aux;
                                
                                    if($diferencia === 1)
                                    {   
                                        print_r('aqui entro');
                                        //GUARDAR SALIDAS HASTA QUE SE IGUAL A ENTRADAS
                                        $datahorasort = $day['day'] .' ' .$day['end_time'];
                                        //GUARDAMOS EN MARCAJES POR RELLENO
                                        $sqls = 'INSERT INTO marcatges_dompliment (id_emp,entsort,datahora,datareg,send) 
                                                VALUES (' .$id .',1,"' .$datahorasort .'", now(),0)';
                                        $this->getDb()->executarSentencia($sqls);

                                        //GUARDAMOS EN MARCAJES
                                        $tipus = 4;
                                        $inout = 1;
                                        $sqls_marcaje = 'INSERT INTO marcatges (id_emp,id_tipus,entsort,datahora,observacions,datareg)
                                                    VALUES ('.$id.','.$tipus.','.$inout.',"'.$datahorasort.'","'.$observacions.'",now())';
                                        $this->getDb()->executarSentencia($sqls_marcaje);
                                        $salidas_aux++;
                                    } else {

                                        //HACEMOS MARCAJES DE UN MINUTO DESPUES SI NO ES LA ULTIMA SALIDA

                                        //IDENTIFICAMOS ULTIMA SALIDA
                                        $marcajes_entradas = [];
                                        foreach($day['marcajes'] as $marcaje_entrada)
                                        {
                                            if($marcaje_entrada['entsort'] == 0) $marcajes_entradas[] = $marcaje_entrada;
                                        }

                                        $ultimo_marcaje_entrada = end($marcajes_entradas);

                                        foreach($day['marcajes'] as $key => $marcaje)
                                        {
                                            if ($marcaje['day'] != $ultimo_marcaje_entrada['day'] && $marcaje['entsort'] == 0 && $day['marcajes'][$key + 1]['entsort'] == 0)
                                            {
                                                $nuevaHora = strtotime('+1 minute', strtotime($marcaje['day']));
                                                $nuevo_marcaje = date('Y-m-d H:i:s', $nuevaHora);

                                                //GUARDAR SALIDAS HASTA QUE SE IGUAL A ENTRADAS
                                                $datahorasort = strval($nuevo_marcaje);
                                                //GUARDAMOS EN MARCAJES POR RELLENO
                                                $sqls = 'INSERT INTO marcatges_dompliment (id_emp,entsort,datahora,datareg,send) 
                                                    VALUES (' .$id .',1,"' .$datahorasort .'", now(),0)';
                                                $this->getDb()->executarSentencia($sqls);
                                ...
                                while($entradas_aux < $salidas)
                                {
                                    //HACER MARCAJE QUE ES, SOLO CUANDO NO ES EL PRIMER MARCAJE, DE RESTO TODOS IGUAL
                                    $primer_marcaje = $day['marcajes'][0];
                                    if($primer_marcaje['entsort'] === 1)
                                    {
                                        //GUARDAR ENTRADAS HASTA QUE SEA IGUAL A SALIDAS
                                        $datahoraent = $day['day'] .' ' .$day['start_time'];
                                        //GUARDAMOS EN MARCAJES POR RELLENO
                                        $sqle = 'INSERT INTO marcatges_dompliment (id_emp,entsort,datahora,datareg,send) 
                                                VALUES (' .$id .',0,"' .$datahoraent .'", now(),0)';
                                        $this->getDb()->executarSentencia($sqle);

                                        //GUARDAMOS EN MARCAJES
                                        $tipus = 4;
                                        $inout = 0;
                                        $sqle_marcaje = 'INSERT INTO marcatges (id_emp,id_tipus,entsort,datahora,observacions,datareg)
                                                        VALUES ('.$id.','.$tipus.','.$inout.',"'.$datahoraent.'","'.$observacions.'",now())';
                                        $this->getDb()->executarSentencia($sqle_marcaje);
                                        $entradas_aux++;
                                    } else {

                                        foreach($day['marcajes'] as $key => $marcaje)
                                        {
                                            if ($marcaje['entsort'] == 1 && $day['marcajes'][$key + 1]['entsort'] == 1)
                                            {
                                                $nuevaHora = strtotime('+1 minute', strtotime($marcaje['day']));
                                                $nuevo_marcaje = date('Y-m-d H:i:s', $nuevaHora);
                                                
                                                //GUARDAR ENTRADAS HASTA QUE SEA IGUAL A SALIDAS
                                                $datahoraent = strval($nuevo_marcaje);
                                                //GUARDAMOS EN MARCAJES POR RELLENO
                                                $sqle = 'INSERT INTO marcatges_dompliment (id_emp,entsort,datahora,datareg,send) 
                                                    VALUES (' .$id .',0,"' .$datahoraent .'", now(),0)';
                                                $this->getDb()->executarSentencia($sqle);
                            ...
                            $sql_verificar = 'SELECT * FROM no_marcatges WHERE id_emp =' . $id .' AND DATE(datahora) =' ."'$day_aux'";
                            $verificar = [];
                            $verificar = $this->getDb()->executarConsulta($sql_verificar);
    
                            if(count($verificar) === 0)
                            {
                                $observacions = 'Relleno por falta de marcaje';

                                //GUARDAMOS A LOS NO MARCAJES
                                $hora = '0:00';
                                $datahora = $day['day'] .' ' .$hora;
                                $sql_nomarcaje = 'INSERT INTO no_marcatges (id_emp,datahora,datareg,send)
                                VALUES (' . $id .', "' . $datahora .'", now(), 0)';
                                $this->getDb()->executarConsulta($sql_nomarcaje);
		...
		$this->mailerConfigIndex($idemp, $fecha_actual);
    }

    public function mailerConfigIndex($idemp, $fecha_actual)
    {
		
		$fecha_anterior = $fecha_actual->format('Y-m-d');
		
        //TRAIGO EMAIL DE RRHH
		$email_to_send = '';
        $sql_email = 'SELECT emailindex FROM empresa WHERE idempresa =' .$idemp;
        $rs = $this->getDb()->executarConsulta($sql_email);
        foreach ($rs as $item) $email_to_send = $item['emailindex'];
		
		$sql_verificar = [];
		$sql_verificar = 'SELECT dni,nom,cognom1,cognom2, datahora, entsort FROM marcatges_dompliment as md JOIN empleat as e ON e.idempleat = md.id_emp WHERE DATE(datareg) = "'.$fecha_anterior.'" AND md.send = 0';
       	$verificar = $this->getDb()->executarConsulta($sql_verificar);
		
		$sql_nomarcajes = 'SELECT dni,nom,cognom1,cognom2,datahora FROM no_marcatges as nm JOIN empleat as e ON e.idempleat = nm.id_emp WHERE nm.send = 0';
        $no_marcajes = $this->getDb()->executarConsulta($sql_nomarcajes);
	
         if($email_to_send && (count($verificar) > 0 || count($no_marcajes) > 0))
        {   
        
            //TRAIGO MARCAJES RELLENOS
            $marcajes_relleno = [];
            $sql = 'SELECT dni,nom,cognom1,cognom2, datahora, entsort FROM marcatges_dompliment as md JOIN empleat as e ON e.idempleat = md.id_emp WHERE DATE(datareg) = "'.$fecha_anterior.'" AND md.send = 0';
            $marcajes_relleno = $this->getDb()->executarConsulta($sql);

            // CREAMOS TABLA HTML CON LOS MARCAJES DE RELLENO
            $tabla = "
            ...
            foreach($marcajes_relleno as $marcaje)
            {
                // Supongo que las columnas y los nombres de las columnas pueden variar según tu estructura de base de datos
                $dni = $marcaje['dni'];
                $nombre = $marcaje['nom'] .' ' .$marcaje['cognom1'] .' ' .$marcaje['cognom2'];
                $dia_hora = $marcaje['datahora'];
                if($marcaje['entsort'] == 0) $tipo = 'Entrada'; 
                if($marcaje['entsort'] == 1) $tipo = 'Salida'; 
            ...
            foreach($no_marcajes as $no_marcaje)
            {
                // Supongo que las columnas y los nombres de las columnas pueden variar según tu estructura de base de datos
                $dni = $no_marcaje['dni'];
                $nombre = $no_marcaje['nom'] .' ' .$no_marcaje['cognom1'] .' ' .$no_marcaje['cognom2'];
                $dia_hora = $no_marcaje['datahora'];

                // Agregar cada fila de datos a la tabla
                $tabla_no_marcajes .= "<tr>
                            <td>$dni</td>
                            <td>$nombre</td>
                            <td>$dia_hora</td>
                    </tr>";
            ...
            try {
                ...
                $mensaje = "
                    ...
                    <p style='font-size: 18px; font-weight: bold;'>Marcajes de Relleno:</p>
                    $tabla

                    <br><br>
                    <p style='font-size: 18px; font-weight: bold;'>Marcajes no realizados dependiendo de sus respectivoso horarios:</p>
                    $tabla_no_marcajes
                ";
		...
		//ACTUALIZO A SEND ESTADO 1
			$taula = 'marcatges_dompliment';
			$camp = 'send';
			$valor = 1;
			$sql_update = 'update '.$taula.' set '.$camp.'=("'.$valor.'") where DATE(datareg) = "'.$fecha_anterior.'" ';
			$this->getDb()->executarSentencia($sql_update);
		...
		//ACTUALIZO A SEND ESTADO 1
			...
			$camp = 'send';
			$valor = 1;
			$sql_update = 'update '.$taula.' set '.$camp.'=("'.$valor.'") where send = 0';
			$this->getDb()->executarSentencia($sql_update);
			
    }
 
    public function horarioOturnoEmpleat ($id, $any, $lng, $fecha) {
        ...
        //TRAIGO LOS DIAS QUE DEBE TRABAJAR EL EMPLEADO (ROTACIO) CON SU RESPECTIVO HORARIO
        $sql = "
            SELECT r.idrotacio as idrotacio, r.data as data, tt.nom as nom, tt.horaentrada as horaEntrada, tt.horasortida as horaSalida, tt.horestreball, tt.horespausa, tt.colorbckg as colorbckg, tt.colortxt as colortxt, tt.abrv as abrv 
            FROM rotacio AS r
                LEFT JOIN tipustorn AS tt ON r.idtipustorn = tt.idtipustorn
            WHERE r.idempleat = $id AND YEAR(r.data) = $any
            ORDER BY r.data
        ...
        foreach ($rs as $valor) {
            foreach ($daysOfYear as &$item)
            {
                if ($valor["data"] === $item["day"])
                {
                    $item["start_time"] = $valor["horaEntrada"];
                    $item["end_time"] = $valor["horaSalida"];
                    $item["horespausa"] = $valor["horespausa"];
                }
            }
        }
        ...
        if((!empty($rs_holiday))&&($prifestius==1))
        {
            ...
            {  
                foreach ($daysOfYear as &$item)
                {
                    ...
                    if ($dateHoliday === $dateCalendari && $any == $anyActual  ) $item["start_time"] = ''; $item["end_time"] = '';
        ...
        if((!empty($rs_holiday))&&($prifestius==0)&&($marcafestius==1))
        {
            foreach ($rs_holiday as $valor)
            {
                foreach ($daysOfYear as &$item)
                {
                    if ($item['day'] === $valor["days"]) $item["start_time"] = ''; $item["end_time"] = '';
                }
            }
        }

        //ELIMINO TODOS LOS ELEMENTOS QUE NO TIENEN NADA O NO PERTENECEN AL HORARIO
        foreach($daysOfYear as $key => $day)  if($day["start_time"] === '' && $day["end_time"] === '') unset($daysOfYear[$key]);

        return $daysOfYear;
    }
	
	public function permittedMailRRHH($idempresa)
    {
        $permitted = [];
        $sql = 'SELECT checkEmailIndex FROM empresa WHERE idempresa =' .$idempresa;
        $checkEmail = $this->getDb()->executarConsulta($sql);
        foreach($checkEmail as $item) $permitted = $item['checkEmailIndex'];
        return $permitted;
    }

    private function nominaEmpleat($month, $any)
    {
        $nomina_pdf_ruta = [];
        $sql = 'SELECT * FROM nominas_cargadas WHERE month = ' .$month .' AND any = ' .$any;
        $nomina_pdf_ruta = $this->getDb()->executarConsulta($sql);
        return $nomina_pdf_ruta;
    }

    public function sendMailNomina ($idsubempresa, $month, $any) {
        $employees = $this->getIdEmployees($idsubempresa);
        
        $nomina_pdf_ruta = $this->nominaEmpleat($month, $any);
        ...
        foreach($employees as $employee)
        {
            ...
            $nomina_pdf = '';
            //ALISTAMOS NOMIMA PDF
            foreach($nomina_pdf_ruta as $nomina)
            {
            ...
            if($email !== null && $email !== '' && $nomina_pdf !== '')
            {
                ...
                try {
                    ...
                    <p style='font-size: 16px;'> ¡Gracias por utilizar nuestro servicio!</p>
                    ";
    ...
    public function cargarNomina($id_emp,$month,$any,$nom,$route,$send)
    {
        $sql = "INSERT INTO nominas_cargadas (id_emp, month, any, nom, route, send)
        VALUES ('$id_emp', '$month', '$any', '$nom', '$route', '$send')";
        $this->getDb()->executarSentencia($sql);
    }

    public function getValidarMarcaje($any, $id)
    {
        //CREO UN ARRAY CON LOS ULTIMOS 12 MESES A PARTIR DE LOS PARAMETROS ANY Y MES

        $data = [];

        // Obtener el timestamp para el primer día del mes y año proporcionados
        $fecha = strtotime("$any-01-01");

         // Generar la información para los últimos 4 meses
        for ($i = 0; $i < 12; $i++) {
            // Calcular el año y mes para el mes actual del bucle
            $any_actual = date('Y', $fecha);
            $mes_actual = date('m', $fecha);

            //TRAIGO LAS NOMINAS CARGADAS DEL EMPLEADO POR MES Y AÑO
            $nomina_cargada = [];
            $nomina_cargada = $this->getCargaNominaById($id, $mes_actual, $any_actual);

            $route_nomina = null;
            if(!empty($nomina_cargada)) $route_nomina = $nomina_cargada[0]['route'];

            //TRAIGO LAS VALIDACIONES REALIZADAS
            $validaciones = [];
            $validaciones = $this->getValidacionesById($id, $mes_actual, $any_actual);

            $validacion = 0;
            $dataregValidacion = '';
            if(!empty($validaciones)) 
            {
                $validacion = $validaciones[0]['validacion'];
                $dataregValidacion = $validaciones[0]['datareg'];
                $dataregValidacion_datetime = new Datetime($dataregValidacion);
                $dataregValidacion = $dataregValidacion_datetime->format('Y-m-d');
            }

            $mes_now = date('m');
            $dia_now = date('d');
            $mes_now = intval($mes_now);
            if($mes_now > $mes_actual || ($mes_now == $mes_actual && $dia_now > 4)) {
                $route_marcaje = "EmpleatInformValidaMarcatge.php?setmana=Todas&id=$id&any=$any_actual&mes=$mes_actual";
                $target = "_blank";
            }
            else {
                $route_marcaje = "ValidarMarcaje.php?id=$id&noinforme=$mes_actual";
                $target = "_self"; // Cambiar el valor de target a "_self" para abrir la página en el mismo navegador.
            }
            
            // Crear el array y agregarlo a la matriz
            $data[] = [
                'any' => $any_actual,
                'mes' => $mes_actual,
                'route_marcaje' => $route_marcaje,
                'validar' => $validacion,
                'route_nomina' => $route_nomina,
                'dataregValidacion' => $dataregValidacion,
                'target' => $target
            ];
    ...
    public function getCargaNominaById ($idempleat, $month, $any)
    {
        return $this->getDb()->executarConsulta("SELECT * FROM nominas_cargadas WHERE id_emp=$idempleat AND month=$month AND any=$any");
    }

    public function getValidacionesById ($id, $month, $any) 
    {
        return $this->getDb()->executarConsulta("SELECT * FROM validaciones_marcajes WHERE id_emp=$id AND mes=$month AND any=$any");
    }

    public function marcajeMesValidado($id, $mes, $any)
    {
        return $this->getDb()->executarConsulta("SELECT * FROM validaciones_marcajes WHERE id_emp=$id AND mes=$mes AND any=$any");
    }

    public function traerNombreEmpleado ($id){
        return $this->getDb()->executarConsulta("SELECT nom FROM empleat WHERE idempleat=$id ");
