...
<body class="well">
    ...
    if (!isset($_GET["any"])) $_GET["any"] = date('Y', strtotime("now"));
    ...
    foreach ($rspers as $rp) {
        if (($i == 0) && ($id != $rp["idempleat"])) {
            $idpersant = $rp["idempleat"];
        }
        if (($i > 0) && ($id == $rp["idempleat"])) {
            $persant = '<button class="btn-neutro" name="id" value="' . $idpersant . '" formaction="AdminFitxaEmpleat.php" title="' . $dto->__($lng, "Anterior") . '"><span class="glyphicon glyphicon-arrow-left"></span></button>';
        } else {
            $idpersant = $rp["idempleat"];
        }
        $persopt .= '<option value="' . $rp["idempleat"] . '">' . $rp["cognom1"] . ' ' . $rp["cognom2"] . ' ' . $rp["nom"] . '</option>';
        if ($chkpers == 1) {
            $persseg = '<button class="btn-neutro" name="id" value="' . $rp["idempleat"] . '" formaction="AdminFitxaEmpleat.php" title="' . $dto->__($lng, "Següent") . '">
        ...
        <div class="row">
            <div class="col-lg-12">
                ...
                <div class="col-lg-2">
                    ...
                    <input type="text" id="searchInput" onkeyup="searchSelect()" onkeydown="return event.key !== 'Enter'" placeholder="Buscar Empleado">
    ...
    <div class="row mb-4 flex justify-content-between m-2">

        <!-- ROW DE VALIDAR MARCAJES Y OPCIONES DE IR A PESTAÑA MARCAJES,FICHA,CALENDARIO.... -->
            ...
            <div class="row">
                ...
                <div class="col">
                    ...
                    <div class = "custom-select-container-año glass-container-año">
                        <select class="glass-select-año" name="validar_marcajes_año">
                            <option hidden selected value="<?php echo $any_actual; ?>"><?php echo $any_actual; ?></option>
                ...
                <div class="col">
                    ...
                    <div class = "custom-select-container glass-container">
                        <select class="glass-select" name="validar_marcajes">
                            <option hidden selected value="<?php echo $mes; ?>"><?php echo $mes_texto; ?></option>
                            <?php foreach($meses as $key => $mes_aux): ?>
                                <option value="<?php echo $key; ?>"><?php echo $mes_aux; ?></option>
                            <?php endforeach; ?>
                        </select>
                        <span class="select-arrow"><i class="fas fa-chevron-down"></i></span>
                    </div>
    ...
    <div class="row">
        <div class="col-lg-12">
            ...
            <div class="col-lg-12" style="text-align: center">
                <h2 class=""><?php echo $dto->mostraNomEmpPerId($id); ?> </h2><br>
                ...
                <div class="col-lg-12">
                            ...
                            <tr>
                                ...
                                <td>
                                    <select onchange="actualitzaCampTaulaNR(' . "'empleat','sexe'," . $id . ',' . "'" . $fitxa["sexe"] . "'" . ',this.value);">
                                        ...
                                        <option value="h">' . $dto->__($lng, "Hombre") . '</option>
                                        <option value="d">' . $dto->__($lng, "Mujer") . '</option>
                                    </select>
                                </td>
                            </tr>';
                                                }
                    ...
                    <tr>
                     ...
                     <tr>
                    <th>' . $dto->__($lng, "Observaciones") . '</th>
                    <td>
                        <textarea
                            style="width: 100%;"
                            onblur="actualitzaCampTaulaNR(' . "'empleat','observaciones'," . $id . ',this.getAttribute(' . "'data-old_value'" . '),this.value);"
                            data-old-value="' . $fitxa["observaciones"] . '"
                        >' . $fitxa["observaciones"] . '</textarea>
                    </td>
                </tr>
                    ...
                    ';
                            ...
                            <tr>
                                ...
                                //mandar variable a la funcion editavacancesassignat para realizar comparacion
       ...
       // Consulta SQL para obtener el total de días del año actual
                                $sql_actual = "SELECT total_dias,dias_any_anterior FROM vacances WHERE idempleat = ? AND any = ?";
                                $stmt_actual = $conn->prepare($sql_actual);
                                $stmt_actual->bind_param("ss", $id_empleat, $current_year);
                                $stmt_actual->execute();
                                $stmt_actual->bind_result($total_dias,$total_dias_any_anterior);
                                                ...
                                                <table class="glass-table table-hover">
                                                    <tbody class="col-lg-4">
                                                        ...
                                                        try {
                                                            ...
                                                            foreach ($rstr as $tr) {
                                                                $tipustornopt .= '<option value="' . $tr["idtipustorn"] . '">' . $tr["nom"] . '</option>';
                                                            }
                    ...
                    <tr><th>' . $dto->__($lng, "Horari Setmanal") . '</th><td>' . $dto->mostraNomHorariPerIdhorari($idhorari) . '</td><td>';
                                                                if (empty($dto->seleccionaHorarisEmpPerId($fitxa["idempleat"], $any))) {
                                                        ...
                                                        try {
                                                            ...
                                                            foreach ($rs as $fitxa) {
                                                                ...
                                                                '<tr><th>' . $dto->__($lng, "Horari R. Principal") . '</th><td colspan="2">' .
                                                                    '<select onchange="updateTurnoRotacionalFicha(' . "'empleat','idhorari1'," . $fitxa["idempleat"] . ',this.value);" style="width: 100%;">'
                                                                    ...
                                                                    '<select onchange="updateTurnoRotacionalFicha(' . "'empleat','idhorari2'," . $fitxa["idempleat"] . ',this.value);" style="width: 100%;">'
                                                                    . '<option hidden selected value="' . $fitxa["idhorari2"] . '">' . $dto->getCampPerIdCampTaula("tipustorn", $fitxa["idhorari2"], "nom") . '</option>'
                                                                    . $tipustornopt . '</select>' . '</td></tr>';

                                                                
                                                                    try {
                                                                        $any = date('Y'); // Año actual
                                                                        $optany = '';
                                                                        for ($jear = 2017; $jear <= (date('Y') + 1); $jear++) {
                                                                            $selected = $jear == $any ? 'selected' : '';
                                                                            $optany .= '<option value="' . $jear . '" ' . $selected . '>' . $jear . '</option>';
                                                                        }
                                                                        $horesasgany = 0.0;
                                                                        $rsha = $dto->getDb()->executarConsulta('select * from horesany where idempleat=' . $id . ' and anylab=' . $any);
                                                                        if (!empty($rsha)) {
                                                                            foreach ($rsha as $ha) {
                                                                                $horesasgany = $ha["hores"];
                                                                            }
                                                                        }
                                                                        echo '<tr><form name="horesany" method="GET"><input hidden name="id" value="' . $id . '"><th>' . $dto->__($lng, "Hores a Realitzar Any") . ' <select name="any" onchange="document.getElementById(\'selectedYear\').value=this.value; this.form.submit();"><option hidden selected value="' . $any . '">' . $any . '</option>' . $optany . '</select></th><td>' . number_format($horesasgany, 1, ",", ".") . ' Hs.</td></form><td><button class="btn-next" title="' . $dto->__($lng, "Edita") . '" onclick="editaHoresAny(' . $fitxa["idempleat"] . ',horesany.any.value);"><span class="glyphicon glyphicon-pencil"></button></td></tr>';
                                                                    } catch (Exception $ex) {
                                                                        echo $ex->getMessage();
                                                                    }
                                                                ...
                                                                if ($fitxa["enplantilla"] == 0) {
                                                                    $datacessat = '<tr><th style="background-color: red; color: red">' . $dto->__($lng, "Cessat") . ' ' . $dto->__($lng, "a") . ' ' . $dto->__($lng, "Data") . '</th><td><input type="date" onchange="actualitzaCampTaulaNR(' . "'empleat','datacessat'," . $id . ',this.getAttribute(' . "'data-old_value'" . '),this.value);"'
                                                                        . 'value="' . $fitxa["datacessat"] . '" data-old_value="' . $fitxa["datacessat"] . '"></td></tr>';
                                                                }

                                                                 echo '<tr><th>' . $dto->__($lng, "Total días vacaciones asignados año actual")    . '</th><td>' . $total_dias . ' días  </td><td><button class="btn-next" title="' . $dto->__($lng, "Assigna Nou") . '" onclick="editaVacancesAssignat(' . $fitxa["idempleat"] . ' , '.$any.'   );"><span class="glyphicon glyphicon-pencil"></button></td></tr>';

                                                                echo '<tr><th>' . $dto->__($lng, "Total días vacaciones año anterior") . '</th><td>' . $total_dias_any_anterior . ' días </td><td><button class="btn-next" title="' . $dto->__($lng, "Assigna Nou") . '" onclick="editaVacancesAssignat(' . $fitxa["idempleat"] . ' ,'.$any_anterior .'    );"><span class="glyphicon glyphicon-pencil"></button></td></tr>';
    ...
    <tr><th>' . $dto->__($lng, "Fecha Finalización Contrato") . '</th><td><input type="date" onblur="actualitzaCampTaulaNR(' . "'empleat','date_cont_end'," . $id . ',this.getAttribute(' . "'data-old_value'" . '),this.value);"'
                                                                . 'value="' . $fitxa["date_cont_end"] . '" data-old_value="' . $fitxa["date_cont_end"] . '" style=""></td></tr>
    ...
    <div class="modal fade" id="modValidarMarcaje" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true"></div>
    ...
    <script>
        ...
        function editaVacancesAssignat(idempleat,any) {
            var xmlhttp = new XMLHttpRequest();
            xmlhttp.onreadystatechange = function() {
                if (this.readyState === 4 && this.status === 200) {
            ...
            xmlhttp.open("GET", "ModalVacances.php?id=" + idempleat + "&any=" +  any, true);
            xmlhttp.send();
        }
    ...
    <form method="get" class="text-right">
        ...
        <!-- <button type="button" class="btn-red btn-supersmall" name="id" value="<?php echo $id; ?>" title="Enviar información de usuario y contraseña a todos los empleados" onclick="confirmAction()"><span class="glyphicon glyphicon-lock"></span></button> -->

        <script>
            function confirmAction() {
                var confirmation = confirm("¿Desea enviar información de usuario y contraseña a todos los empleados?");
                if (confirmation) {
                    // Si el usuario hace clic en "Aceptar", redirige al script process_correo_user_all.php
                    window.location.href = "process_correo_user_all.php?id=<?php echo $id; ?>";
                }
                // Si el usuario hace clic en "Cancelar", no se ejecuta nada
            }
            ...
            function searchSelect() {
                ...
                select = document.getElementById("sizesel");
            ...
            const showModalValidarMarcaje = (idempleat) => {
                const selectElement = document.querySelector('select[name="validar_marcajes"]');
                ...
                xmlhttp.onreadystatechange = function() {
                    if (this.readyState === 4 && this.status === 200) {
                        document.getElementById("modValidarMarcaje").innerHTML = this.responseText;
                        $modal = $('#modValidarMarcaje');
                        $modal.modal('show');
                    }
                };
                xmlhttp.open("GET", "ModalValidarMarcaje.php?id=" + idempleat + "&mes=" + mes + "&any=" + any, true);
                xmlhttp.send();
