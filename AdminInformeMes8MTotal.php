...
<center>
    <div class="row">
        ...
        <div class="col-lg-8" id="print" style="border: solid 1px; border-radius: 3px;">
            ...
            if (isset($_GET['any']) && isset($_GET['mes'])) {
                ...
                $sql = "
                    SELECT e.idempleat, e.nom, e.cognom1, e.cognom2,
                        ...
                        TIME(MAX(CASE WHEN t1.entsort = 0 AND t1.row_number = 4 THEN t1.datahora END)) AS entrada4,
                        TIME(MAX(CASE WHEN t1.entsort = 1 AND t1.row_number = 4 THEN t1.datahora END)) AS salida4,
            ...
            function calcularTotalHoras($entries, $exits,$id,$fecha) {
                ...
                if ($entries[1] && $exits[1]) {
                    $entrada1_segundos = strtotime($entries[1]);
                    $salida1_segundos = strtotime($exits[1]);
                    $totalSegundos += $salida1_segundos - $entrada1_segundos;
                }

                if ($entries[2] && $exits[2]) {
                    $entrada1_segundos = strtotime($entries[2]);
                    $salida1_segundos = strtotime($exits[2]);
                    $totalSegundos += $salida1_segundos - $entrada1_segundos;
                }

                if ($entries[3] && $exits[3]) {
                    $entrada1_segundos = strtotime($entries[3]);
                    $salida1_segundos = strtotime($exits[3]);
                    $totalSegundos += $salida1_segundos - $entrada1_segundos;
                }

                if ($entries[4] && $exits[4]) {
                    $entrada1_segundos = strtotime($entries[4]);
                    $salida1_segundos = strtotime($exits[4]);
                    $totalSegundos += $salida1_segundos - $entrada1_segundos;
                }
                    ...
                    <div style="margin-top:30px" id="contingut">
                        ...
                        <table border="1"  class="table table-bordered" style="text-align: center; font-size: 12px; border-collapse: collapse; width: 100%">
                            ...
                            <tr style="background-color: white; color: black">
                                ...
                                <th rowspan="2" style="text-align: center; align-content: center; border: solid 1px;"><?php echo $dto->__($lng, "ENTRADA 4"); ?></th>
                                <th rowspan="2" style="text-align: center; align-content: center; border: solid 1px;"><?php echo $dto->__($lng, "SALIDA 4"); ?></th>
                            ...
                            while ($fechaInicio <= $fechaFin) {
                                ...
                                foreach ($idtreballador['rows'] as $row) {
                                    ...
                                    if ($diaActual == $fechaRegistro) {
                                        ...
                                        //FILTRO PARA NO PERMITIR LAS SALIDAS MAS TEMPRANAS QUE LAS ENTRADAS, Y LAS ENTRADAS MAS TARDES QUE LA SALIDA
                                        for ($i = 1; $i <= 4; $i++)
                                        {
                                            if ($i>2)
                                            {
                                                if ($entries[$i] < $exits[$i-1]) $entries[$i] = '';
                                                if ($exits[$i] < $entries[$i]) $exits[$i] = '';
                                            }
                                        }
                                        ...
                                        $salida1 = !$exits[1] && $entries[1] ? "<span style='color: red;'>Sin marcar salida</span>" : $exits[1];
                                        $salida2 = !$exits[2] && $entries[2] ? "<span style='color: red;'>Sin marcar salida</span>" : $exits[2];
                                        $salida3 = !$exits[3] && $entries[3] ? "<span style='color: red;'>Sin marcar salida</span>" : $exits[3];
                                        $salida4 = !$exits[4] && $entries[4] ? "<span style='color: red;'>Sin marcar salida</span>" : $exits[4];

                                        $entrada1 = !$entries[1] && $exits[1] ? "<span style='color: blue;'>Sin marcar entrada</span>" : $entries[1];
                                        $entrada2 = !$entries[2] && $exits[2] ? "<span style='color: blue;'>Sin marcar entrada</span>" : $entries[2];
                                        $entrada3 = !$entries[3] && $exits[3] ? "<span style='color: blue;'>Sin marcar entrada</span>" : $entries[3];
                                        $entrada4 = !$entries[4] && $exits[4] ? "<span style='color: blue;'>Sin marcar entrada</span>" : $entries[4];

										$fechaRegistroData = $row['fecha'];

                                        // Puedes usar las variables $row['entrada1'], $row['salida1'], etc., para acceder a los datos del registro.
                                        $data_time = [];
                                        $data_time= calcularTotalHoras($entries, $exits,$id,$fechaRegistroData);
                                ...
                                $totalMinutos_diferencia_2 = [];
     							 ...
     							 // quito ultimos ceros para que quede formato horas - minutos
                                        ...
                                        //quito ultimos ceros para que quede formato horas - minutos
                                        $salidas = [$salida1, $salida2, $salida3, $salida4];                                       
                                        $salida_formato = [];
                                        ...
                                        echo "<td>" . $entrada_formato[2] . "</td>";
                                        echo "<td>" . $salida_formato[2] . "</td>";
                                        echo "<td>" . $entrada_formato[3] . "</td>";
                                        echo "<td>" . $salida_formato[3] . "</td>";
  $horasOrdinarias = ($horasRegistradas ? $data_time['totalFormato'] : "0:00");
                                        $horasVoluntarias = ($horasDiferencia ? $diferenciaHoras['totalFormatoDiferencia'] : "0:00");
                                        ...
                                        echo "<td>" . $horasOrdinarias  . "</td>";

                                        echo "<td>" . $horesteoriques . "</td>";
                                        echo "<td>" . $horasVoluntarias . "</td>";

								
                                         //TRAIGO LAS EXCEPCIONES Y SI NO HAY NADA MUESRTA LAS OBSERVACIONES
                                $data1 = "";
