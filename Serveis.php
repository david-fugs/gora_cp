...
switch ($_GET["action"])
{
    ...
    case "sendMailRRHH":
        ...
        break;
		    ...
		    case "añadirMasArchivos":
        session_start();
        $dto->addMasArchivos($_GET["carpeta"], $_FILES);
        break;

		case "borrarArchivo":
        session_start();
        $dto->borrarArchivo($_GET["carpeta"], $_POST['namefile']);
        break;
    ...
    case "actualitzaCampTaula":
        ...
        break;
		 ...
		 case "updateTurnoRotacionalFicha":
            $msg = "";
            try{
            $dto->updateTurnoRotacionalFicha($_GET["taula"],$_GET["camp"],$_GET["idreg"],$_GET["valor"]);
            //print_r([$_GET["taula"],$_GET["camp"],$_GET["idreg"],$_GET["valor"]]);
            } catch (Exception $ex) {
                http_response_code(404);
                $msg = $ex->getMessage();
            }
		...
		case "crearDepartament":

        $msg = "";
        try{
        $dto->crearDepartament($_GET["nomdpt"],$_GET["idempresa"],$_GET["ambit"]);
        } catch (Exception $ex) {
            http_response_code(404);
            $msg = $ex->getMessage();
        }
		...
		case "actualitzaFestiu":
            $msg = "";
            try{
            $response = $dto->actualitzaFestiu($_GET["taula"],$_GET["camp"],$_GET["idreg"],$_GET["valor"]);
