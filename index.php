...
<head>
    ...
    <style>
        ...
        @media (max-width: 850px) {
            ...
            #hr_before {
                margin-top: 5px !important; margin-bottom: 50px !important;
            }
    ...
    $d=strtotime("today");
	    ...
	    //PERMITIR EL ENVIO DE MAIL AL RRHH
        $permittedMailRRHH = '';
        $permittedMailRRHH = $dto->permittedMailRRHH(7);
    ?>

</head>
<body onload="startTime(<?php echo $day.",".$month.",".$year;?>);automarcatges(<?php echo $idempresa;?>);carregaInicial(<?php echo $idempresa;?>);sendMailRRHH(<?php echo $permittedMailRRHH;?>)" style="display: table; width: 100%; background-size: cover">




<h2 id="datadia"><?php
    echo '<div style="margin-left: 53px;" class="diaHora">' . $dto->__($lng, $dto->mostraNomDia((date("w", $d)))) . ", " . abs(date("d", $d)) . " " . $dto->__($lng, $dto->mostraNomMes(date("m", $d))) . " " . (date("Y", $d)) . "" . "  " . '<span id="hora"></span></div>';
    ?>
</h2>

<div style="width: 100%; text-align: center;">
    <hr id="hr_before" style="border: 0.1px solid #d3d3d3; width: 95%; margin-top: 10px; margin-bottom: 100px;">
</div>
...
<div class="row container" id="main_container" style="flex-direction: row; display: flex; justify-content: center; align-content: center; width: 100%; background-size: cover ;">

    <div class="col-xs-4" id="first_col" style="align-content: center;">
        <center>
...
<table>
    <tbody>';
        ...
        <tr>
            <td style="height: 55px; width: 55px; text-align: center">
                <input class="imagen" type="image" id="entr3" entsort="0" title="'. $dto->__($lng, "Tornada de") . ' ' . $dto->__($lng, $dto->getDescidActivitat($dto->getLogoidRealitza($idempresa, 3))) .'" style="cursor:pointer" src="'. $dto->getLogoIn($idempresa, $dto->getLogoidRealitza($idempresa, 3)) .'" value="'. $dto->getidActivitatidRealitza($idempresa, 3) .'" width="58px">
            </td>
    ...
    <div class="custom-select-container">
        <button class="well-sm custom-select-button" title="' . $dto->__($lng, "Totes les Entrades") . '">
            <div class="custom-select-arrow"></div>
        </button>
        <select id="selE">
            <option disabled selected>' . $dto->__($lng, "Totes les Entrades") . '</option>';
    ...
    <div class="col-xs-1" id="div_logo" style="display:flex; justify-content: center; align-items: center; padding-left: 20px; padding-right: 20px; margin-left: 20px; margin-right: 20px;">
        <image class="imagen-transparente" src="./Pantalles/img/logo_grupo_minim.png">
    </div>
...
<table>
    ...
    <tbody>
        ...
        <tr>
            <td style="height: 55px; width: 55px; text-align: center">
                <input class="imagen" type="image" id="entr3" entsort="1" title="' . $dto->__($lng, "Sortida per") . ' ' . $dto->__($lng, $dto->getDescidActivitat($dto->getLogoidRealitza($idempresa, 3))) . '" style="cursor:pointer" src="' . $dto->getLogoOut($idempresa, $dto->getLogoidRealitza($idempresa, 3)) . '" value="' . $dto->getidActivitatidRealitza($idempresa, 3) . '" width="58px">
            </td>
            <td>' . $dto->__($lng, $dto->getNomidRealitza($dto->getLogoidRealitza($idempresa, 3))) . '</td>
        </tr>';
            endif;
        ...
        <tr>
            ...
            echo '<div class="custom-select-container">
        <button class="well-sm custom-select-button-red" title="' . $dto->__($lng, "Totes les Sortides") . '">
            <div class="custom-select-arrow"></div>
        </button>
        <select id="selE">
            <option disabled selected>' . $dto->__($lng, "Totes les Sortides") . '</option>';
