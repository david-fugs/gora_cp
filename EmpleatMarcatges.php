...
?>
    <script type="text/javascript">
        ...
        function mostraInformeMesAuto(id,any,mes,setmana)
        {
            window.location = "AdminInformeMesAuto.php?id=" +id+ "&any=" +any+ "&mes=" +mes+ "&setmana=" +setmana;
        }
		...
		function mostraInformeMes8M(id,any,mes,setmana)
        {
            window.location = "AdminInformeMes8M.php?id=" +id+ "&any=" +any+ "&mes=" +mes+ "&setmana=" +setmana;
        }
...
$marcajeMesValidado = [];
$marcajeMesValidado = $dto->marcajeMesValidado($id, $mes, $any);
$showMesValido = false;
if(!empty($marcajeMesValidado)) $showMesValido = true;
                ...
                <div class="">
                    <div class="row">
                        <div class ="col-lg-12" style = "margin-top: 20px">
                            ...
                            <?php if($showMesValido) : ?> 
                                <div class="alert alert-success" role="alert" id="success_carga_masiva" style="width: 500px;">
                                    Marcaje del mes  <?php echo $mes; ?> verificado
                                </div>
                            <?php endif ?>
                            ...
                            <div class="col-lg-1">
                                ...
                                <form action="EmpleatMarcatges.php" method="GET">
...
<div class="row" style="margin-top: 40px">
    <div class = "col-lg-3"  style ="text-align: left; margin-left: 50px;">
        ...
        <h4 id="calendar"><?php $dto->paintCalendarMonth($mes, $month_calendar, $lng); ?></h4>
        <div class="row"></div>
        <script>
    var calendar = document.getElementById('calendar');
    if (calendar) {
        calendar.querySelectorAll('*[onclick]').forEach(function(element) {
        ...
        <p><strong><?php echo $dto->__($lng, "Año") . ": " . number_format($horesteoany, 2, ",", "."); ?> h </strong></p>
                ...
                for ($d = 0; $d < $numdiesvistos; $d++) {
                    ...
                    if ((empty($marcatgesdia)) && ($dto->seleccionaHoresTeoriquesPerIdDia($id, $diesvistos[$d]) > 0)) {
                        ...
                        foreach ($marcatgesdia as $marcatge) {
                                ...
                                switch ($weekday) {
                                    case $i:
                                        if ($marcatge["validat"] == 1) {
                                            ...
                                            $bodymarcatges .= '<td>'
                                                . '<label style="border: solid 1px; border-radius: 5px; padding: 3px; background-color: ' . $bckcol . '; cursor: pointer" " title="' . $dto->__($lng, $dto->direntsort($marcatge["entsort"])) . ": " . $dto->__($lng, $marcatge["tipus"]) . '&NewLine;' . $dto->__($lng, "Click per a editar") . '">' . $icon . ' ' . $hora . '</label>'
                            ...
                            //Problema con la ubicacion desde el marcaje, lo hice para que si es de aranda ponga aranda, si no coloque lo que viene del marcaje
                            if (!empty($marcatge["utm_x"])) {
                               
                                if($idsubemp == 13  ){
                                    
                                $tdloc = '<td title="' . $dto->__($lng, "Veure Mapa") . '"><a style="cursor: pointer;" onclick="window.open(' . "'https://www.google.com/maps/search/?api=1&query=" . $marcatge["utm_x"] . "," . $marcatge["utm_y"] . "'" . ');">' .  "Aranda". ' <span class="glyphicon glyphicon-map-marker" style="color: orangered"></span></a></td>';
...
//OPERACION APARTE PARA ALGO MAS ABAJO
                    ...
                    for ($j = 1; $j <= 6; $j++) {
                        ...
                        $deficit = differenceHours($worked_hours[$j], $theoretical_hours[$j]);
                         ...
                         if (strpos($deficit, '-') !== false) {
                            // Si contiene -, se aplica el fondo rojo
                            echo '<td style="background-color:rgb(255,128,128)">' . $deficit . '</td>';
                        } else {
                            // Si no contiene -, se aplica el fondo verde
...
<div class="modal fade" id="modInforme" role="dialog"></div>
