...
function updateTurnoRotacionalFicha(taula,camp,idreg, valornou)
{    
    var xmlhttp = new XMLHttpRequest();
    xmlhttp.onreadystatechange = function() {
    if (this.readyState === 4 && this.status === 404) {
    ...
    xmlhttp.open("GET", "Serveis.php?action=updateTurnoRotacionalFicha&taula=" + taula + "&camp=" + camp + "&idreg=" + idreg  +  "&valor=" + valornou, true);
    xmlhttp.send();
}



function ActualizaFestiu(taula,camp,idreg,valorvell,valornou)
{
    if(valorvell === valornou) return false;    
    var xmlhttp = new XMLHttpRequest();
    xmlhttp.onreadystatechange = function() {
    ...
    xmlhttp.open("GET", "Serveis.php?action=actualitzaFestiu&taula=" + taula + "&camp=" + camp + "&idreg=" + idreg + "&valor=" + valornou, true);
    xmlhttp.send();
}
...
function gestionaRefrescaHoraris(idemp)
{
    ...
    setTimeout(function(){ 
        ...
        setTimeout(function() {
            location.reload();
        }, 300); // Cambia el tiempo a tu preferencia
            },900);
...
function eliminaHorari(idhorari, idemp) {
    ...
    xmlhttp.onreadystatechange = function() {
        if (this.readyState === 4 && this.status === 200) {
            ...
            location.reload(); // Recargar la página después de eliminar el horario
        }
        if (this.readyState === 4 && this.status === 404) {
            popuphtml(this.responseText);
        }
...
function afegeixUbicacio(nom) {
    ...
    xmlhttp.onreadystatechange = function() {
        if (this.readyState === 4 && this.status === 200) {
            location.reload(); // Recarga la página si la solicitud es exitosa
        }
        if (this.readyState === 4 && this.status === 404) {
            popuphtml(this.responseText);
        }
...
{        
    ...
    xmlhttp.open("GET", "ModEditaExcepcio.php?&1="+idexcep+"&2="+tipus+"&3="+dataini+"&4="+datafi, true);
    xmlhttp.send();
}
...
function validar_marcaje_desde_fitxa(validar,any,mes,id)
{
    if(validar == 0)
    {
        var xmlhttp = new XMLHttpRequest();
